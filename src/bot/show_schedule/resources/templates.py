...
{% for lesson in schedule | sort(attribute='start_time') -%}
    Пара {{ convert_time_from_utc(lesson.start_time, timezone).strftime('%H:%M')}}-{{end_time(lesson.start_time, timezone).strftime('%H:%M') }} {% if has_classrooms %}в <em>{{ lesson.classroom }}</em>{% endif %}\n
