...
def show_choose_period_buttons() -> InlineKeyboardMarkup:
    ...
    builder.button(text="Сегодня", callback_data=ScheduleDayCallbackData(chosen_day=date.today()))
...
def show_choose_day_buttons(week_to_add: int = 0) -> InlineKeyboardMarkup:
    ...
    day_of_week = date.today() + timedelta(days=7*week_to_add)
    week_runner = day_of_week - timedelta(days=day_of_week.weekday() % 7)
    for i in range(7):
        builder.button(text=week_runner.__str__(), callback_data=ScheduleDayCallbackData(chosen_day=week_runner))
