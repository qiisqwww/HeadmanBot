...
Это класс который описывает один (UseCase, Command). Например, пользователю нужно получить
посещаемость для студента, то для этого можно написать GetStudentAttendanceUseCase или GetStudentAttendanceCommand.  

Ограничения:
- Использовать репозитории из другого контекста можно (их интерфейсы)
- Использовать в одном UseCase другой максимально нежелательно, только в крайнем случае (DRY или SRP не в счет, он них здесь можно забыть).
Пример такого пограничного случая - логика, которая должна выполняться в другом поддомене. Например, регистрация пользователя. Она в 
себя включает: создание пользователя, группы и посещамость с расписанием. С пользователем все ок, он в нашем поддомене, с группой тоже,
там нет особой логики, просто вызов репозитория, но с посещаемостью и расписанием все не так, все куда сложнее. Если написатб всю эту логику
в одном UseCase (RegistrateStudentCommand), то мы будем в student_management выполнять работу attendance модуля. Лучше создание расписания и 
посещаемость вынести в CreateAttendanceCommand в attendance модуле и вызывать ее.
